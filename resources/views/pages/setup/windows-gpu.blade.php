				...
				<div class="card-content">
					<div class="content">
						<ol>
							...
							<li>Extract the archive into <code>C:\DaggerGpuMiner</code></code></li>
							<li>Download <code>win64exe.zip</code> wallet from the <a href="http://xdag.me/downloads.html" target="_blank">official website</a>.</li>
							<li>Extract the archive into <code>C:\DaggerGpuMiner\win64exe</code></code></li>
							<li>Create a new bat file <code>C:\DaggerGpuMiner\win64exe\RUNWALLET.bat</code></li>
							<li>Edit with notepad. Insert one line into the file: <pre class="oneline">C:\DaggerGpuMiner\win64exe\xdag.exe -d -m 1 pool.xdagpool.com:13654</pre>
							<li>Run the <code>RUNWALLET.bat</code> file.</li>
							<li>Set your wallet password, type random keys (at least 3 lines or random keys). Wait until the host keys are generated (might take a while).</li>
							...
							<li>Navigate to <code>C:\DaggerGpuMiner\win64exe</code>. You will see a folder called <code>storage</code>. Copy this folder one level up, into <code>C:\DaggerGpuMiner</code></li>
							<li>Create a new bat file <code>C:\DaggerGpuMiner\RUNMINER.bat</code></li>
							<li>Edit with notepad. Insert one line into the file: <pre class="oneline">C:\DaggerGpuMiner\xdag.exe -G -a "<span class="parameter">wallet_address</span>" -p pool.xdagpool.com:13654 -t 0 -v 2 -opencl-platform <span class="parameter">platform_id</span> -opencl-devices <span class="parameter">device_nums</span></pre> Replace <span class="parameter">wallet_address</span> with the address you copied into notepad. Replace <span class="parameter">platform_id</span> with your OpenCL platform ID, this is most usually 0 or 1. If you have more than one GPU in the system, count up from zero to (number of devices - 1), so for example if you have 4 GPUs in your system, replace <span class="parameter">device_nums</span> with 0 1 2 3. If you have only one GPU, replace <span class="parameter">device_nums</span> with 0. To see advanced GPU miner parameters, execute <code>DaggerGpuMiner.exe -h</code>.</li>
				...
				<div class="card-content">
					<div class="content">
						<ol>
							...
							<li>To view your current balance at any time, execute <code>RUNWALLET.bat</code><li>, type in your wallet password, press enter, type <code>balance</code> and press enter. If you see <code>not ready to show balance</code>, wait a bit and type <code>balance</code> again, followed by enter key. When you are done, type <code>terminate</code> followed by enter to clsoe your wallet. You can also check your balance at any time by using our website, or registering your miner.</li>
						</ol>
					</div>
				</div>
			</nav>
