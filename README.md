...
2. install mysql 5.7 or mariadb. Create new database, for example `pool`, with `CREATE DATABASE openxdagpool CHARACTER SET utf8mb4 COLLATE utg8mb4_unicode_ci` run as mysql's `root` user. Grant all privileges: `GRANT ALL ON openxdagpool.* TO pool@'%' IDENTIFIED BY 'PWD!!!!'`. Choose your own password!
